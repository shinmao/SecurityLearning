...
2. bypass with **PHP String escape sequences** and **PHP Variable functions**  
PHP has several ways to represent string, we can [some tricks hidden in double quoted](https://secure.php.net/manual/en/language.types.string.php)  
For example, waf blocks the string of `system`:  
```php
// system("ls")
"\x73\x79\x73\x74\x65\x6d"("ls")
```  
In the wrap of double quotes, `\x[0-9A-Fa-f]{1,2}` would parse character in hexadecimal notation.  
Furthermore, if `"` is also blocked by waf, we can use round bracket to represent string:  
```php
...
echo (a);
```  
**whatever inside round brackets without a specific type declaration, is treated as a string**.  

3. bypass with `get_defined_functions`  
```php
